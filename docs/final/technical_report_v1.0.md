...
当前正式版本采用 integrated suite `0001-0009`。该序列统一提供 `hello`、`add2`、`pstate`、`pcount`、`fcount`、`pgcount`、`fdcount`、`memstat` 和 `fdinfo`，syscall 编号为 22 到 30。rain、root、z2996 三方 full verification 已登记为 PASS，最终演示视频和 SHA256 已记录在 `submissions/evidence_manifest.md`。historical `0001-0007` 只作为历史稳定检查点。
...
最终综合序列从项目脚本声明的 clean xv6 baseline 出发，按 `0001` 到 `0009` 应用 patch。baseline 元数据由 `scripts/xv6/apply-integrated-labs.sh` 和 `external/xv6-baseline-record.md` 维护，报告正文只说明复现入口。
...
当前正式 `integrated 0001-0009` 已由 rain、root、z2996 三方 full verification PASS，并由 `grade-summaries` 解析为 3 clean PASS。最终视频 `20260611_final_integrated_0001_0009_demo.mp4` 覆盖该验证范围，SHA256 已登记。视频、截图、summary 原件和 raw logs 不进入 Git，只在 evidence manifest 中记录元数据和哈希。
...
本报告中的完成状态必须能追溯到 `patches/`、`scripts/`、`docs/final/` 或 `submissions/evidence_manifest.md`。涉及验证结果时，应同时说明 suite、命令和证据位置。涉及教学价值时，应说明对应 lab、概念和学生可观察现象。
...
本文内容限定在当前标题所对应的项目记录、教学说明、复现步骤或审查结论内。涉及当前正式验证范围、历史证据、验证命令和证据材料时，应以 `submissions/evidence_manifest.md`、`patches/integrated-labs/README.md` 和相关脚本为准。

## 结构规范

文档应按“背景或问题、过程或设计、证据或命令、风险和后续动作”的顺序组织。历史文档可保留阶段性记录，但必须避免覆盖当前正式验证范围。
