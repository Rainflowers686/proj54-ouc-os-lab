...
当前正式版本采用 integrated suite `0001-0009`。syscall 编号为 hello=22、add2=23、pstate=24、pcount=25、fcount=26、pgcount=27、fdcount=28、memstat=29、fdinfo=30。rain、root、z2996 三方 full verification 已登记为 PASS。
...
本项目不是 LTP 覆盖项目，不实现完整内存管理、文件系统或调度器。Lab5 不新增内核机制。historical `0001-0007` 只作为历史稳定检查点。
