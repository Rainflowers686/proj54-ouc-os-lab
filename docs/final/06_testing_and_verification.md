...
| teammate-verify | 队友正式复现入口 | `bash scripts/xv6/teammate-verify.sh --full` | 队友机器 summary overall PASS（现含 memstattest/fdinfotest） | historical：`e8e2fb9` root 与 z2996 两份 `0001-0007` full PASS 已记录；旧 `1ba9db6` 为 historical evidence。TBD：`0001-0009`（含 memstat/fdinfo）的 rain/root/z2996 full verify 尚未进行，不得伪造 | `submissions/teammate_reproduction_record.md`，`submissions/evidence_manifest.md` |
| manual xv6 shell demo | 人工进入 xv6 shell 运行 hello/add2/pstate/pcount/pchild/fcount/pgcount/fdcount/memstat/fdinfo | `cd external/xv6-riscv && make qemu` 后手动输入用户程序 | 真实 shell 输出，不依赖 timeout 自动捕获 | historical：`0001-0007` 视频已记录大小、时长、SHA256，降级为 historical stable checkpoint，只覆盖 `0001-0007`；旧三段视频为 historical evidence。TBD：覆盖 `0001-0009`（含 memstat/fdinfo）的新视频与新 SHA256 尚未录制；平台提交方式和隐私复核待最终确认 | `submissions/demo_record.md`，`submissions/evidence_manifest.md` |
...
- Lab3 当前已进入 integrated `0006`，Lab4 v0.2 `fdcount()` 已进入 integrated `0007`；stage11b 进阶 `memstat` 进入 integrated `0008`、`fdinfo` 进入 integrated `0009`，final integrated suite 扩展为 `0001-0009`。旧 `1ba9db6` 队友 PASS 不覆盖 `e8e2fb9`；`e8e2fb9` 三方 full PASS 又只覆盖 `0001-0007`、降级为 historical stable checkpoint；`0001-0009` 的队友 full verify 为 TBD，只作占位、不得伪造。
