...
> 维护时间：2026-06-06（stage9a）。
> 本文档只做设计和风险评审，不修改 xv6 内核、不新增 patch、不修改 `scripts/xv6/`。
...
| lab3 memory/pagetable | 未完成 | 目前只有规划/占位，不能写成已完成 |
...
当前不能写“Lab0-Lab5 全部完成”。准确说法应为：lab0/lab1/lab2/lab4 已完成，lab3 和独立 lab5 仍待补全；integrated-labs `0001-0005` 已完成并验证。

两位队友 full verify PASS 锚定在 commit `1ba9db6 tooling: speed up verification and clean repo hygiene`。后续如果修改 patch 序列、`scripts/xv6/` 验证入口或新增 Lab3 到一键验证，现有队友 PASS 不能直接覆盖新状态，需要重新复现。
...
推荐做 `pgcount()` + `pgcounttest`，定位为“页表映射数量观察实验”。
