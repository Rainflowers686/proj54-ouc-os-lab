...

{
  ...
  status_line "docs/23_teammate_quickstart.md" "teammate quickstart" "stage7a1: plain-language one-shot reproduction instructions; git pull, teammate-verify, cleanup-qemu, feedback format, common warnings, and no external/logs/secrets"
  status_line "submissions/submission_checklist.md" "submission checklist" "stage7a1 pre-submission checklist: completion status, must-fill items, teammate one-shot workflow, forbidden items, pre-submit commands, and honest-MVP conclusion"
  ...
  status_line "scripts/xv6/cleanup-qemu.sh" "xv6 QEMU cleanup helper" "stage7a1 rescue tool: lists qemu/make qemu processes, explains Ctrl+C vs Ctrl+Z, runs pkill cleanup, exits 0"
  status_line "scripts/xv6/teammate-verify.sh" "teammate one-shot verification" "stage7a1 workflow: check-env, baseline, apply+make, boot, hello/add2/pstate/pcount/pchild/fcount verification, PASS/FAIL summary in ignored logs"
  ...
  echo "- scripts/xv6/teammate-verify.sh is the recommended teammate one-shot workflow; it writes summary files under ignored logs/."
  ...
  echo "- Plain-language teammate quickstart is documented in docs/23_teammate_quickstart.md."
} > "$out"
