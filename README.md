...
| 队友一键复现 | 已新增 stage7a1 | `scripts/xv6/teammate-verify.sh` 自动执行环境、apply+make、boot 和 7 个命令验证；summary 写入 ignored 的 `logs/` |
...
它会自动运行环境检查、baseline 检查、integrated `0001-0005` apply+make、boot evidence，以及 hello/add2test/pstatetest/pcounttest/pchildtest/fcounttest 验证。最后会打印 summary，并写入 ignored 文件：
