...
19. 测试报告：`docs/04_test_report.md`
20. AI 使用记录：`docs/05_ai_usage_record.md`
21. 引用与许可证说明：`docs/08_reference_and_license.md`
22. 初赛材料索引：`submissions/draft-report-index.md`
...
| integrated lab1+lab2 sequence | 已形成 patch 并验证 | `patches/integrated-labs/` 可从 clean baseline 顺序应用 `0001-0004`；同一构建中已捕获 hello/add2test/pstatetest/pcounttest/pchildtest 输出 |
...
patches/integrated-labs/      lab1+lab2 综合演示 patch sequence，当前包含 0001-0004
...
7. 综合演示：从 clean baseline 应用 integrated patch sequence。
   ...
   说明：`apply-integrated-labs.sh` 默认预览，不修改 external tree；`--make --yes` 会 reset/clean ignored 的 `external/xv6-riscv/`，应用 integrated `0001-0004` 并运行 `make`。子进程观察程序实际命令名为 `pchildtest`，这是为了避开 xv6 `DIRSIZ` 文件名长度限制；输出仍为 `pstate(child) = ...`。
